...
Документ описывает единые правила проектирования и эволюции REST JSON контрактов между n8n-флоу.
 ...
 ## TL;DR чеклист
  - [ ] Версионирование и пути: `/{businessFlow}/{tool}/v{major}/{action}` (например, `/lead-routing/hubspot-sync/v1/create-export`); все сегменты в kebab-case; минорные изменения — back‑compat.
  ...
  - [ ] Идемпотентность без хранения: используйте `PUT /{businessFlow}/{tool}/v1/<resource>/{deterministicId}` или повторяемые операции; ретраи `POST` не поддерживаются.
 ...
 - Длительные операции: немедленный ответ `202 Accepted` с `jobId` и последующий `GET /{businessFlow}/{tool}/v1/jobs/{jobId}` (статус) и/или `.../download`.
...
### 3.1 Шаблон пути
- **Шаблон**: `/{businessFlow}/{tool}/v{major}/{action}`
...
- Для идемпотентного создания/обновления без хранения предпочтительны детерминированные ID:
  - Пример: `PUT /{businessFlow}/{tool}/v1/exports/{deterministicId}`
- Используйте `409 CONFLICT` для дубликатов с различающимися данными.

### 3.4 Длительные операции
- Старт: `POST /{businessFlow}/{tool}/v1/{action}` → `202 Accepted` с `jobId`.
- Поллинг: `GET /{businessFlow}/{tool}/v1/jobs/{jobId}` → статус.
- Скачивание (если применимо): `GET /{businessFlow}/{tool}/v1/jobs/{jobId}/download`.
...
- Дополнительные вложенные сегменты используйте только при необходимости для ресурсов:
  - Пример: `/{businessFlow}/{tool}/v1/resources/{resourceId}`.
...
- **Версионирование/Путь**: соответствует `/{businessFlow}/{tool}/v{major}/{action}`; все сегменты в kebab-case; указана целевая версия (`v1`/`v2`).
